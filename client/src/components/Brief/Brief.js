...
const Brief = props => {
  ...
  const { id } = props.user;
  if (!isEditContest) {
    return <ContestInfo userId={id} contestData={contestData} changeEditContest={changeEditContest} role={role} goChat={goChat} />;
  } else {
    return (
      <div className={styles.contestForm}>
        {error && <Error data={error.data} status={error.status} clearError={clearUpdateContestStore} />}
        <ContestForm contestType={contestData.contestType} defaultData={getContestObjInfo()} submitData={setNewContestData} />
        {isEditContest ? (
...
const mapStateToProps = state => {
  ...
  return { updateContestStore, user, isEditContest };
};
