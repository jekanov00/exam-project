...
import { goToExpandedDialog, changeChatFavorite, changeChatBlock, changeShowAddChatToCatalogMenu } from '../../../../actions/actionCreator';
...
const DialogList = props => {
  ...
  const getTimeStr = time => {
    ...
    else if (currentTime.isSame(time, 'week')) return moment(time).format('dddd');
    else if (currentTime.isSame(time, 'year')) return moment(time).format('MM DD');
  ...
  const renderChatPreview = () => {
    ...
    else if (chatMode === CONSTANTS.BLOCKED_PREVIEW_CHAT_MODE) return renderPreview(onlyBlockDialogs);
